...
Our plan for $n \geq 5$ is as follows: given games of a particularly nice form, we will show how to construct a game of one greater length while still preserving this niceness property. The result is that this construction can be repeated indefinitely until a game of the desired length is reached. First, we need the following lemma:

\begin{lemma}
Suppose the game $(a, b, c, d)$ has length $n$. Then the games $(2a, 2b, 2c, 2d)$ and $(a + k, b + k, c + k, d + k)$ for any $k\in \mathbb{Z}$ also have length $n$.
\end{lemma}

\textit{Proof:} Suppose the $(a, b, c, d)$ game is $\Big((a_i, b_i, c_i, d_i)\Big)_{i=0}^{n - 1}$. Then it is clear that the $(2a, 2b, 2c, 2d)$ game is $\Big((2a_i, 2b_i, 2c_i, 2d_i)\Big)_{i=0}^{n - 1}$. Similarly, the difference of $(a + k, b + k, c + k, d + k)$ is the same as that of $(a, b, c, d)$. The result follows.
...
By the lemma above, this game also has length $n$. Furthermore, since each element of this tuple is non-negative, it is the difference of the following sequence:

$$(0, c - b - a, 2a + 2(c - b - a), 2a + 2b + 3(c - b - a))$$

This game thus has length $n + 1$ since its difference yields a game of length $n$. Note also that this tuple is again of the form $(0, a', b', c')$, where $0\leq a'\leq b'\leq c'$ and $a' + b' = 2a + 3(c - b - a) < 2a + 2b + 3(c - b - a) = c'$. Thus we may iterate this process to yield a game of length $n + 2$, and so on.
...
The proof of this follows very easily from the fact that game length is invariant under the operation of scaling each element of the $4$-tuple by a non-zero quantity. We used this fact in our construction of arbitrarily long games in Section $\ref{sec:longgames}$.
