...
In particular, we are interested in repeatedly applying $\diff$ to an initial tuple, until the tuple (0, 0, 0, 0) is reached. We call the (possibly infinite) sequence of tuples obtained by repeatedly applying $\diff$ to $(a, b, c, d)$  the \textit{game on} $(a, b, c, d)$ or, where the context is clear, simply $(a, b, c, d)$.

For example, the game on $(3, 7, 2, 5)$ is:
\begin{align*}
& (3, 7, 2, 5) \\
& (4, 5, 3, 2) \\
& (1, 2, 1, 2) \\
...
We will now show that every integer game terminates at the $4$-tuple $(0,0,0,0)$ in finite time (actually, as we will see, very quickly).
...
Equipped with these simplifications, we can now embark upon a proof of Theorem \ref{thm:pow2}.
...
\begin{proof}
Let $n = \lfloor\log_2{M}\rfloor + 1$. By Theorem \ref {thm:pow2}, $D^{4n}((a, b, c, d))$ has elements that are congruent to $0\pmod{2^n}$. However, since $M < 2^n$, all of the elements in $D^{4n}((a, b, c, d))$ are also less than $2^n$, and thus are all zero.
\end{proof}
